...
use Squareetlabs\LaravelTeamsPermissions\Support\Services\AuditService;
use Squareetlabs\LaravelTeamsPermissions\Events\TeamMemberAdding;
use Squareetlabs\LaravelTeamsPermissions\Events\TeamMemberInvited;
use Squareetlabs\LaravelTeamsPermissions\Events\TeamMemberAdded;
use Squareetlabs\LaravelTeamsPermissions\Events\TeamMemberInviting;
use Squareetlabs\LaravelTeamsPermissions\Events\TeamMemberRemoved;
use Squareetlabs\LaravelTeamsPermissions\Events\TeamMemberRemoving;
use Squareetlabs\LaravelTeamsPermissions\Events\TeamMemberUpdated;
use Squareetlabs\LaravelTeamsPermissions\Mail\Invitation;
...
trait HasMembers
{
    ...
    public function users(): BelongsToMany
    {
        ...
        return $this->belongsToMany(TeamsFacade::model('user'), TeamsFacade::model('membership'), $teamIdField)
            ->withPivot('role_id')
            ->withTimestamps()
            ->as('membership');
    }
    ...
    {
        $teamIdField = Config::get('teams.foreign_keys.team_id');
        return $this->hasMany(TeamsFacade::model('ability'), $teamIdField, 'id');
    }
    ...
    {
        $teamIdField = Config::get('teams.foreign_keys.team_id');
        return $this->hasMany(TeamsFacade::model('role'), $teamIdField, 'id');
    }
    ...
    {
        $teamIdField = Config::get('teams.foreign_keys.team_id');
        return $this->hasMany(TeamsFacade::model('group'), $teamIdField, 'id');
    }
    ...
    {
        $teamIdField = Config::get('teams.foreign_keys.team_id');
        return $this->hasMany(TeamsFacade::model('invitation'), $teamIdField, 'id');
    }
    ...
    public function addUser(object $user, string $role_keyword): void
    {
        ...
        $auditService->logTeamMemberAdded($this, $this->owner, $user, $role);
    }
    ...
    public function updateUser(object $user, string $role_keyword): void
    {
        ...
        // Log audit event
        $auditService = new AuditService();
        $auditService->logRoleAssigned($this, $this->owner, $role);
    }
    ...
    public function deleteUser(object $user): void
    {
        ...
        // Log audit event
        $auditService = new AuditService();
        $auditService->logTeamMemberRemoved($this, $this->owner, $user);
    }
    ...
    public function inviteUser(string $email, int|string $keyword): void
    {
        ...
        $key = 'team-invite:' . $this->id;
        $maxAttempts = Config::get('teams.invitations.rate_limit.max_attempts');
        $decaySeconds = Config::get('teams.invitations.rate_limit.decay_seconds');

        if (RateLimiter::tooManyAttempts($key, $maxAttempts)) {
            throw new RuntimeException("Too many invitations sent. Please try again later.");
        }
     ...
     * @return void
     */
    protected function invalidateTeamCache(): void
    {
        if (!Config::get('teams.cache.enabled')) {
            return;
        }

        $cache = new \Squareetlabs\LaravelTeamsPermissions\Support\Services\PermissionCache();
        $cache->flush();
    }
    ...
    public function getPermissionIds(array $codes): array
    {
        ...
        // Get existing permissions (permissions are global, not team-specific)
        ...
        if (!empty($newPermissions)) {
            $items = array_map(fn ($code) => ['code' => $code], $newPermissions);
            
            $permissionModel::query()->insert($items);
